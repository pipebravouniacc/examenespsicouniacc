...
export function Disponibilidad({ profesorId, titulo }) {
  ...
  // Replica una misma franja en todas las fechas habilitadas (precarga rápida)
  const replicar = async (ini, fin) => {
    setMsg(null)
    if (!ini || !fin || fin <= ini) {
      setMsg({ t: 'error', x: 'La hora de término debe ser posterior a la de inicio.' })
      return
    }
    const nuevas = ventanas
      .filter((v) => !disps.some((d) => d.fecha === v.fecha && d.hora_inicio.slice(0, 5) === ini && d.hora_fin.slice(0, 5) === fin))
      .map((v) => ({ profesor_id: profesorId, fecha: v.fecha, hora_inicio: ini, hora_fin: fin }))
    if (!nuevas.length) {
      setMsg({ t: 'info', x: 'Esa franja ya estaba en todas las fechas.' })
      return
    }
    const { error } = await supabase.from('disponibilidades').insert(nuevas)
    if (error) setMsg({ t: 'error', x: error.message })
    else setMsg({ t: 'ok', x: `Franja agregada en ${nuevas.length} fecha(s).` })
    cargar()
  }
  ...
  return (
    <div className="card">
      ...
      {ventanas.length > 1 && <ReplicarFranja ventanas={ventanas} onReplicar={replicar} />}
      {ventanas.map((v) => (
        <DiaDisponibilidad
          key={v.id}
          ventana={v}
...
function ReplicarFranja({ ventanas, onReplicar }) {
  const [ini, setIni] = useState('09:00')
  const [fin, setFin] = useState('18:00')
  return (
    <div className="dia-card" style={{ background: 'var(--soft)' }}>
      <div className="fecha">Aplicar una misma franja a las {ventanas.length} fechas</div>
      <div style={{ fontSize: 12.5, color: 'var(--gris)' }}>
        Útil para precargar disponibilidad amplia y luego ajustar días puntuales.
      </div>
      <div className="franja">
        <input type="time" value={ini} onChange={(e) => setIni(e.target.value)} aria-label="Desde" />
        <span>a</span>
        <input type="time" value={fin} onChange={(e) => setFin(e.target.value)} aria-label="Hasta" />
        <button className="secundario mini" onClick={() => onReplicar(ini, fin)}>
          Aplicar a todas
        </button>
      </div>
    </div>

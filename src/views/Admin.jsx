...
import { proponer, validarComision, conflictosHorario } from '../lib/scheduler'
import { fmtFecha, fmtFechaLarga, fmtNota, nombreCorto, CARGOS } from '../lib/util'
import { pdfRetroalimentacion, pdfRegistroObservaciones, descargarPDF } from '../lib/pdf'
import { Disponibilidad } from './Docente'
import EditorTesis from './EditorTesis'
...
export default function Admin({ perfil }) {
  ...
  const reprobadosSinReagendar = datos.examenes.filter(
    (e) => e.estado === 'realizado' &&
      (e.estudiantes || []).some((s) => s.aprobado === false && s.apto_rendir !== false) &&
...
function Tesis({ datos, cargar, nom }) {
  ...
  const [editando, setEditando] = useState(null) // objeto examen, o 'nuevo'
  ...
  const importar = async (file) => {
    ...
    try {
      ...
      const { filas, advertencias } = parsearCSV(texto, datos.profesores)
      if (!filas.length) throw new Error('No se encontraron filas con tesis o integrantes en el archivo.')
      const existentes = new Set(datos.examenes.map((e) => e.clave_dedup).filter(Boolean))
      let nuevos = 0, omitidos = 0, agendados = 0, sinGuia = 0
      for (const f of filas) {
        if (existentes.has(f.clave)) { omitidos++; continue }
        const { data: ex, error } = await supabase
          .from('examenes')
          .insert({
            clave_dedup: f.clave,
            codigo: f.codigo,
            nombre_trabajo: f.nombre_trabajo,
            area: f.area,
            guia_texto: f.guia_texto,
            guia_id: f.guia_id,
            presidente_id: f.presidente_id,
            experto_id: f.experto_id,
            presidente_texto: f.presidente_texto,
            experto_texto: f.experto_texto,
            fecha: f.fecha,
            hora_inicio: f.hora_inicio,
            hora_fin: f.hora_fin,
            modalidad: f.modalidad,
            modalidad_carrera: f.modalidad_carrera,
            sala: f.sala,
            registro_obs: f.registro_obs,
            estado: f.estado,
            estado_portal: f.estado_portal,
            resultado_portal: f.resultado_portal,
            apto_rendir: f.apto_rendir,
            raw: f.raw,
          })
          .select()
          .single()
        ...
        nuevos++
        if (f.estado === 'agendado') agendados++
        if (!f.guia_id) sinGuia++
      }
      const partes = [`${nuevos} tesis importadas`]
      if (agendados) partes.push(`${agendados} ya venían con fecha y comisión completa (quedaron agendadas)`)
      if (sinGuia) partes.push(`${sinGuia} con guía sin vincular`)
      if (omitidos) partes.push(`${omitidos} omitidas por estar ya cargadas`)
      setMsg({
        t: advertencias.length || sinGuia ? 'warn' : 'ok',
        x: partes.join(' · ') + (advertencias.length ? ' — ' + advertencias.join(' ') : ''),
      })
      cargar()
    } catch (e) {
  ...
  const exportar = () => {
    ...
    descargarArchivo('Planificacion_examenes_de_grado_actualizada.csv', exportarCSV(datos.examenes, porExamen, byId))
  }
  ...
  if (editando)
    return (
      <EditorTesis
        examen={editando === 'nuevo' ? null : editando}
        datos={datos}
        alCerrar={(guardado) => { setEditando(null); if (guardado) cargar() }}
      />
    )
  ...
  return (
    <div>
      <div className="card">
        ...
        <p className="sub">
          Exporta el List a CSV desde SharePoint y súbelo aquí. Se leen las columnas tal como
          vienen: Nombre de la tesis, Integrantes, las tres columnas de docente guía, presidente,
          experto, Fecha Examen, Hora Inicio, Sala, Modalidad y Observaciones. Si agregas al List
          las columnas <strong>Área</strong> y <strong>RUT 1</strong> a <strong>RUT 4</strong>,
          también se importan. Las tesis ya cargadas se omiten y el resto de las columnas se
          conservan intactas al exportar.
        </p>
        <div className="acciones">
          ...
          <button className="suave" onClick={exportar}>Exportar CSV</button>
          <button className="secundario" onClick={() => setEditando('nuevo')}>Ingresar tesis manualmente</button>
        </div>
      ...
      <div className="card">
        ...
        {activos.length > 0 && (
          <table className="datos">
            <thead>
              <tr><th>ID</th><th>Tesis / estudiantes</th><th>Área</th><th>Fecha · sala</th><th>Guía</th><th>Estado</th><th></th></tr>
            </thead>
            <tbody>
              {activos.map((e) => (
                <tr key={e.id}>
                    ...
                    {(e.estudiantes || []).some((s) => s.apto_rendir === false) && (
                      <div style={{ marginTop: 3 }}>
                        <span className="chip rojo">
                          no rinde: {(e.estudiantes || []).filter((s) => s.apto_rendir === false).map((s) => s.nombre).join(', ')}
                        </span>
                      </div>
                    )}
                  </td>
                  <td>{e.area || <span style={{ color: 'var(--gris)' }}>sin área</span>}</td>
                  <td style={{ fontSize: 12.5 }}>
                    {e.fecha ? <>{fmtFecha(e.fecha)}<br />{e.hora_inicio?.slice(0, 5)}</> : '—'}
                    {e.sala ? <><br /><span className="chip gris">{e.sala}</span></> : ''}
                  </td>
                  <td>
                    {e.guia_id ? (
                      nom(e.guia_id)
                  ...
                  <td>
                    ...
                    {e.apto_rendir === false && <><br /><span className="chip rojo">nadie habilitado</span></>}
                  </td>
                  <td>
                    <div className="acciones" style={{ marginTop: 0 }}>
                      <button className="secundario mini" onClick={() => setEditando(e)}>Editar</button>
                      <button className="peligro mini" onClick={() => eliminar(e)}>Eliminar</button>
                    </div>
                  </td>
                </tr>
              ))}
...
function Propuestas({ datos, cargar, nom }) {
  ...
  // Cuenta como ocupado tanto lo agendado como las propuestas por confirmar,
  // para que dos rondas de generación no comprometan al mismo docente.
  const ocupaciones = () =>
    datos.examenes
      .flatMap((e) => {
        if (e.estado === 'agendado' && e.fecha)
          return [{
            examen_id: e.id,
        ...
        if (e.estado === 'propuesto' && e.propuesta?.fecha)
          return [{
            examen_id: e.id,
            fecha: e.propuesta.fecha,
            hora_inicio: e.propuesta.hora_inicio,
            hora_fin: e.propuesta.hora_fin,
            guia_id: e.guia_id,
            presidente_id: e.propuesta.presidente_id,
            experto_id: e.propuesta.experto_id,
          }]
        return []
      })
  ...
  const generarTodas = async () => {
    ...
    const pendientes = datos.examenes.filter((e) => e.estado === 'pendiente' && e.apto_rendir !== false)
    let ok = 0
    const sinProp = []
    for (const ex of pendientes) {
      ...
      if (r.ok) {
        ...
        ctx.ocupaciones.push({ ...r.propuesta, guia_id: ex.guia_id, examen_id: ex.id })
        ok++
      } else {
  ...
  const pendientes = datos.examenes.filter((e) => e.estado === 'pendiente' && e.apto_rendir !== false)
  const noAptos = datos.examenes.filter((e) => e.estado === 'pendiente' && e.apto_rendir === false).length
  ...
  return (
    <div>
      <div className="card">
        ...
        {noAptos > 0 && (
          <div className="aviso info">
            {noAptos} tesis no tienen ningún integrante habilitado para rendir y quedan fuera de
            esta ronda. Puedes revisarlo en Tesis → Editar.
          </div>
        )}
      </div>
...
function TarjetaPropuesta({ ex, nom, datos, cargar, ocupacionesGlobal = [], onNueva, onPendiente }) {
  ...
  const confirmar = async (valores) => {
    ...
    const problemas = validarComision(comision, datos.profesores)
  ...
  // Nadie puede ocupar dos roles: se excluye de cada lista a quien ya está en otro
  const disponiblesPara = (rol) => {
    const tomados = [
      ex.guia_id,
      rol === 'presidente' ? m.experto_id : m.presidente_id,
    ].filter(Boolean)
    const base = rol === 'presidente' ? plantas : datos.profesores
    return base.filter((x) => !tomados.includes(x.id))
  }

  const avisosHorario = manual
    ? conflictosHorario(
        { ...ex, ...m },
        ocupacionesGlobal.filter((o) => o.examen_id !== ex.id),
        datos.profesores
      )
    : []
  ...
  return (
    <div className="card">
        ...
        <div className="fila" style={{ marginBottom: 10 }}>
          ...
          <label className="campo"><span>Presidente/a (planta)</span>
            <select value={m.presidente_id} onChange={(e) => setM({ ...m, presidente_id: e.target.value })}>
              ...
              {disponiblesPara('presidente').map((x) => (
                <option key={x.id} value={x.id}>{x.nombre}{x.cargo ? ` · ${x.cargo}` : ''}</option>
              ))}
            </select></label>
          <label className="campo"><span>Docente experto/a</span>
            <select value={m.experto_id} onChange={(e) => setM({ ...m, experto_id: e.target.value })}>
              <option value="">Seleccionar…</option>
              {disponiblesPara('experto').map((x) => (
                <option key={x.id} value={x.id}>{x.nombre} ({x.tipo})</option>
              ))}
            </select></label>
        </div>
      )}
      ...
      {avisosHorario.length > 0 && (
        <div className="aviso warn">
          {avisosHorario.map((x, i) => <div key={i}>{x}</div>)}
          <div style={{ marginTop: 4 }}>
            Puedes confirmar igual si conoces el motivo, pero revisa que no sea un choque real.
          </div>
        </div>
      )}
      <div className="acciones">
...
function Docentes({ datos, cargar, perfil }) {
  ...
  const [cuenta, setCuenta] = useState({
    nombre: '', correo: '', correo_gmail: '', clave: 'Psico2026', tipo: 'planta',
    cargo: 'Académico regular', especialidades: [],
  })
  const [creando, setCreando] = useState(false)

  // Crea la cuenta de acceso vía Edge Function. Si la función no está
  // desplegada, deja la ficha preparada para que el docente se registre.
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('crear-docente', {
        body: {
          ...
          correo_gmail: cuenta.correo_gmail.trim() || null,
          clave: cuenta.clave,
          tipo: cuenta.tipo,
          cargo: cuenta.tipo === 'planta' ? cuenta.cargo || null : null,
          especialidades: cuenta.especialidades,
        },
      ...
      setMsg({
        t: 'ok',
        x: `Cuenta creada para ${cuenta.nombre}. Entrégale el correo y la contraseña ${cuenta.clave}; al ingresar deberá definir la suya.`,
      })
      setCuenta({ ...cuenta, nombre: '', correo: '', correo_gmail: '', especialidades: [] })
      cargar()
    } catch (e) {
      // Alternativa sin Edge Function: ficha preparada, el docente se registra
      const { error: e2 } = await supabase.from('profesores').insert({
        nombre: cuenta.nombre.trim(),
        correo_ms: cuenta.correo.trim(),
        correo_gmail: cuenta.correo_gmail.trim() || null,
        tipo: cuenta.tipo,
        cargo: cuenta.tipo === 'planta' ? cuenta.cargo || null : null,
        especialidades: cuenta.especialidades,
      ...
      if (e2) setMsg({ t: 'error', x: `No se pudo crear la cuenta: ${e.message}. Detalle: ${e2.message}` })
      else {
        setMsg({
          t: 'warn',
          x: `La función de creación de cuentas no está disponible (${e.message}), así que dejé la ficha de ${cuenta.nombre} preparada y habilitada. Pídele que cree su cuenta en la pantalla de inicio con el correo ${cuenta.correo.trim()}: quedará vinculada automáticamente con sus especialidades y horarios.`,
        })
        setCuenta({ ...cuenta, nombre: '', correo: '', correo_gmail: '', especialidades: [] })
        cargar()
      }
  ...
  // Elimina la ficha del docente. Si participa en algún examen, no se borra:
  // se explica el motivo y se ofrece deshabilitarlo, que preserva el historial.
  const eliminarDocente = async (p) => {
    setMsg(null)
    const usos = datos.examenes.filter(
      (e) => e.guia_id === p.id || e.presidente_id === p.id || e.experto_id === p.id
    )
    if (usos.length) {
      setMsg({
        t: 'warn',
        x: `${p.nombre} participa en ${usos.length} examen(es) y por eso no puede eliminarse sin perder ese registro. ` +
           'Si ya no forma parte del cuerpo docente, deshabilítalo/a: deja de aparecer en las propuestas y conserva el historial.',
      })
      return
    }
    if (!window.confirm(
      `¿Eliminar definitivamente la ficha de ${p.nombre}? Se borrarán también sus horarios cargados.`
    )) return
    try {
      // Si la Edge Function está desplegada, elimina además su cuenta de acceso
      if (p.auth_id) {
        const { data: s2 } = await supabase.auth.getSession()
        await supabase.functions.invoke('crear-docente', {
          body: { accion: 'eliminar', correo: p.correo_ms },
          headers: s2?.session ? { Authorization: `Bearer ${s2.session.access_token}` } : undefined,
        }).catch(() => null)
      }
      const { error } = await supabase.from('profesores').delete().eq('id', p.id)
      if (error) throw error
      setMsg({ t: 'ok', x: `Ficha de ${p.nombre} eliminada.` })
      cargar()
    } catch (e) {
      setMsg({ t: 'error', x: `No se pudo eliminar: ${e.message}` })
    }
  }
  ...
  const sinCargo = plantas.filter((p) => !p.cargo).length
  ...
  return (
    <div>
      ...
      <div className="card">
        ...
        {sinCargo > 0 && (
          <div className="aviso warn">
            {sinCargo} docente(s) de planta aún no tienen cargo asignado. Mientras no lo tengan,
            quedan al final de la prioridad para presidir comisiones.
          </div>
        )}
        <table className="datos">
          <thead>
            <tr>
              <th>Nombre</th><th>Cargo</th><th>Correos</th>
              <th>Especialidades</th><th>Estado</th><th></th>
            </tr>
          </thead>
          <tbody>
            {plantas.map((p) => (
              <tr key={p.id}>
                ...
                <td>
                  <select
                    value={p.cargo || ''}
                    onChange={(e) => setCargo(p, e.target.value)}
                    style={{ fontSize: 12.5, minWidth: 160 }}
                  >
                    <option value="">Sin asignar</option>
                    {CARGOS.map((c) => <option key={c} value={c}>{c}</option>)}
                  </select>
                  {!p.cargo && <div><span className="chip pendiente">sin cargo</span></div>}
                </td>
                ...
                <td>
                  <div className="acciones" style={{ marginTop: 0 }}>
                    ...
                    {p.id !== perfil.id && (
                      <button className="peligro mini" onClick={() => eliminarDocente(p)}>Eliminar</button>
                    )}
                  </div>
                </td>
              </tr>
        ...
        <p className="sub" style={{ marginTop: 10 }}>
          Prioridad para presidir comisiones: {CARGOS.join(' → ')}.
        </p>
      </div>

      <div className="card">
        <div className="eyebrow">Alta de acceso</div>
        <h2>Crear cuenta de docente</h2>
        <p className="sub">
          Creas la cuenta con una contraseña provisoria y se la entregas al docente. Al ingresar
          por primera vez, el sistema le pedirá definir la suya propia. Puedes dejar cargadas sus
          especialidades y, con el botón Horarios, también su disponibilidad.
        </p>
        <div className="fila">
          <label className="campo"><span>Nombre completo</span>
            <input value={cuenta.nombre} onChange={(e) => setCuenta({ ...cuenta, nombre: e.target.value })} /></label>
          <label className="campo"><span>Correo (será su usuario)</span>
            <input value={cuenta.correo} onChange={(e) => setCuenta({ ...cuenta, correo: e.target.value })}
              placeholder="nombre.apellido@uniacc.cl" /></label>
        </div>
        <div className="fila">
          <label className="campo"><span>Correo alternativo del List (opcional)</span>
            <input value={cuenta.correo_gmail} onChange={(e) => setCuenta({ ...cuenta, correo_gmail: e.target.value })}
              placeholder="nombre.apellido@uniacc.edu" /></label>
          <label className="campo"><span>Contraseña provisoria</span>
            <input value={cuenta.clave} onChange={(e) => setCuenta({ ...cuenta, clave: e.target.value })} /></label>
          <label className="campo"><span>Tipo</span>
            <select value={cuenta.tipo} onChange={(e) => setCuenta({ ...cuenta, tipo: e.target.value })}>
              <option value="planta">Planta</option>
              <option value="hora">Profesor/a hora</option>
            </select></label>
        </div>
        {cuenta.tipo === 'planta' && (
          <label className="campo" style={{ maxWidth: 320 }}>
            <span>Cargo institucional</span>
            <select value={cuenta.cargo} onChange={(e) => setCuenta({ ...cuenta, cargo: e.target.value })}>
              <option value="">Sin asignar</option>
              {CARGOS.map((c) => <option key={c} value={c}>{c}</option>)}
            </select>
          </label>
        ...
        <div className="check-grid">
          {datos.especialidades.map((e) => (
            <label key={e.id}>
              <input type="checkbox" checked={cuenta.especialidades.includes(e.nombre)}
                onChange={(ev) => setCuenta({
                  ...cuenta,
                  especialidades: ev.target.checked
                    ? [...cuenta.especialidades, e.nombre]
                    : cuenta.especialidades.filter((x) => x !== e.nombre),
                })} />
              {e.nombre}
            </label>
          ))}
        ...
        <div className="acciones">
          <button className="primario" onClick={crearCuenta}
            disabled={creando || !cuenta.nombre.trim() || !cuenta.correo.trim() || cuenta.clave.length < 6}>
...
function Historial({ datos, cargar, nom, byId }) {
  ...
  const reagendar = async (ex) => {
    setMsg(null)
    const reprobados = (ex.estudiantes || []).filter((s) => s.aprobado === false && s.apto_rendir !== false)
    ...
    try {
      ...
      const { error: e2 } = await supabase.from('estudiantes').insert(
        reprobados.map((s, i) => ({
          examen_id: nuevo.id, nombre: s.nombre, rut: s.rut, orden: i + 1, apto_rendir: true,
        }))
      )
      if (e2) throw e2
            ...
            <table className="datos" style={{ maxWidth: 640 }}>
              ...
              <tbody>
                {(ex.estudiantes || []).map((s) => (
                  <tr key={s.id}>
                    <td>{s.nombre}</td>
                    <td className="mono">{s.apto_rendir === false ? '—' : fmtNota(s.nota_presentacion)}</td>
                    <td className="mono">{s.apto_rendir === false ? '—' : fmtNota(s.nota_defensa)}</td>
                    <td>
                      {s.apto_rendir === false ? (
                        <span className="chip gris">No rindió</span>
                      ) : s.aprobado ? (
                        <span className="chip verde">Aprueba</span>
                      ) : (
                        <span className="chip rojo">Reprueba</span>
                      )}
                    </td>
                  </tr>
                ))}

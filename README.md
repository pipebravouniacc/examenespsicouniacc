...
6. **Evaluación**: el día del examen, el presidente entra a **Evaluar exámenes**. La rúbrica está organizada en tres secciones diferenciadas por color: (1) Presentación grupal, (2) Presentación individual y (3) Defensa. En las secciones individuales el indicador se muestra una sola vez y debajo aparece una fila por estudiante con la escala de 1 a 7, de modo que se evalúa el mismo criterio para todo el grupo antes de pasar al siguiente. Los descriptores de cada nivel se despliegan con el botón *Ver niveles*. El sistema calcula las notas con la tabla oficial (ambas deben ser ≥ 4.0) y al cerrar genera los dos PDF.
...
Ambos PDF llevan el logo institucional en el encabezado, sin franjas ni pies de página.

El **PDF de retroalimentación** incluye, además de los puntajes y notas, un bloque de *aspectos a
fortalecer* que se arma automáticamente: por cada criterio que no alcanzó el puntaje 7 se agrega
la conducta esperada en ese indicador, redactada a partir del descriptor de nivel máximo de la
rúbrica oficial. Aparece separado para el componente grupal y para cada estudiante, y quien
obtiene 7 en todos sus criterios recibe una nota de desempeño máximo en lugar del bloque. Los
comentarios que escriba la comisión se agregan al final.

El **Registro de Observaciones** mantiene el formato institucional y solo incluye a quienes
estaban habilitados/as para rendir.

Si en algún momento cambia el logo, basta reemplazar `src/lib/logo.js`, que lo contiene embebido
en base64 para que los PDF no dependan de una conexión externa.

## 9. Tamaño de los grupos

La plataforma no asume un número fijo de estudiantes por tesis. La evaluación, el cálculo de
notas, los PDF y el agendamiento operan con grupos de cualquier tamaño; la rúbrica genera una
fila por estudiante en cada criterio individual y el acta expande sus filas según corresponda.

Los únicos topes son de formulario: el editor manual permite hasta 8 estudiantes por tesis, y si
el List usa columnas desglosadas se leen hasta `Estudiante 8` con sus `RUT 8` y `Apto 8`. La
columna `Integrantes` con nombres separados por barras no tiene límite.

## 10. Cargos y prioridad de presidencia

Cada docente de planta tiene un **cargo institucional**, que se asigna con un desplegable en la
pestaña Docentes:
...
Ese orden es la prioridad para presidir comisiones. Dentro de un mismo cargo, se ubica primero a
quien tiene la especialidad del área de la tesis, y entre iguales el sistema baraja para no
recargar siempre a la misma persona. Las fichas sin cargo asignado quedan al final de la
prioridad, y el panel avisa cuántas faltan por clasificar.

Si prefieren que la especialidad mande por sobre el cargo, basta cambiar en
`src/lib/scheduler.js` la constante `PRIORIDAD_PRESIDENTE` de `'cargo'` a `'especialidad'`. El
resto del motor no cambia.

## 11. Roles nunca repetidos

Guía, presidente y docente experto son siempre tres personas distintas, y esto se sostiene en tres
puntos del sistema. El motor de propuestas descarta cualquier combinación que repita a alguien y
verifica el trío antes de entregar cada propuesta. Los desplegables del agendamiento manual y del
editor de tesis ocultan a quienes ya están en otro rol de ese mismo examen, y además validan antes
de guardar, incluyendo que el presidente sea docente de planta.

El cálculo de disponibilidad considera ocupados tanto los exámenes agendados como **las propuestas
aún por confirmar**, de modo que generar propuestas en varias tandas no compromete dos veces al
mismo docente en el mismo horario. En el agendamiento manual, si la persona elegida ya participa
en otro examen a esa hora, aparece una advertencia que no bloquea la acción, porque la
coordinación puede tener un motivo válido para hacerlo.

## 12. Eliminar docentes

La pestaña Docentes tiene un botón **Eliminar** en cada ficha, tanto de planta como de profesores
hora. Si esa persona participa en algún examen, el sistema no la borra: explica en cuántos aparece
y sugiere deshabilitarla, que la saca de las propuestas conservando el historial de los exámenes
ya realizados. La coordinación no puede eliminar su propia ficha.

Cuando la ficha no tiene exámenes asociados, se elimina junto con sus horarios cargados y, si la
Edge Function está desplegada, también su cuenta de acceso.

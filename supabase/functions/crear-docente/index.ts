...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const {
      accion = 'crear', correo, clave, nombre, tipo = 'planta', especialidades = [],
      cargo = null, correo_gmail = null, habilitado = true,
    } = body

    // --- Eliminar la cuenta de acceso de un docente ---
    if (accion === 'eliminar') {
      if (!correo) return json({ error: 'Falta el correo.' }, 400)
      ...
      return json({ ok: true, eliminado: Boolean(usuario) })
    }
